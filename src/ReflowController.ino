...
#define NAMED_PROFILES      0   // 1 = allow named profiles, 0 = numbers only
...
// this is Number of zero crossings per second
...
volatile Profile_t activeProfile; // the one and only instance
...
// --HELPERS-------------------------------------------------------------------
#define FS(x)      F(x)     // define as F(x) to store strings in flash.
...
{
  tft.setCursor( ((TFT_WIDTH >> 1) - (strlen((const char *)str) * (tft.getTextSize() * 3))) , y);
  tft.print(str);
}
...
bool getItemValueLabel(const Menu::Item_t *mi, char *label)
{
  ...
  if (isPidSetting(mi)) {
    double value;
    if (mi == &miPidSettingI) value = heaterPID.Ki;
    if (mi == &miPidSettingD) value = heaterPID.Kd; 
    if (mi == &miFanSettings) value = fanAssistSpeed;
  
    ftoa(label, value, 2); // need greater precision with pid values
  }
  if (isRampSetting(mi)) {
    
    int16_t value;
    if (mi == &miRampUpRate)  value = activeProfile.rampUpRate;
    if (mi == &miRampDnRate)  value = activeProfile.rampDownRate;

    // set integer
    //itoa10((*((int16_t *)value) / 10), label, 1);
    // set decimal
    //work[0] = (value % 10) + 0x30;
    //work[1] = 0x00;
    //strcat(label, ".");
    //strcat(label, work);
    // set degC/s
    //strcat(label, "\367C/s");
    itostr(label, value, "\367C/s");
  }
  ...
  if (mi == &miPeakTemp || mi == &miSoakTemp) {
      int16_t value;
    if (mi == &miSoakTemp)    value = activeProfile.soakTemp;
    if (mi == &miPeakTemp)    value = activeProfile.peakTemp;  

    itostr(label, value, "\367C");
  }
  if (mi == &miPeakTime || mi == &miSoakTime) {
      int16_t value;
  if (mi == &miSoakTime)    value = activeProfile.soakDuration;
  if (mi == &miPeakTime)    value = activeProfile.peakDuration;
    itostr(label, value, "s");
  }
  
  if (mi == &miFanSettings) {
   
    
    itostr(label, fanAssistSpeed, "%");
  }

  return val;
}*/

/*bool getItemValueLabel2(const Menu::Item_t *mi, char *label)
{
  label = "hello";
  return;
}*/
...
void setup()
{
  ...
  attachInterrupt(INT_ZX, zeroCrossingIsr, CHANGE); //RISING
  delay(100);
#else
 #if defined(__AVR_ATmega328P__)
...
void saveProfile(uint8_t targetProfile, bool quiet)
{
  ...
  if (!quiet) delay(500);
#endif
  /*itoa(activeProfile.soakTemp,buf,10);
  printCentered(buf, TFT_HEIGHT - 41);
  itoa(activeProfile.soakDuration,buf,10);
  printCentered(buf, TFT_HEIGHT - 26);
  //printCentered(FS("karl@pitrich.com"), TFT_HEIGHT - 16);
  delay(2500);*/
}
...
void loadParameters(uint8_t profile)
{
  ...
  //Serial.print(activeProfile.soakDuration);

  
  /*if(activeProfile.soakDuration <0) {
    activeProfile.soakDuration = 0;
  }*/
